...
export function ChatResponseComponent({ response }: ChatResponseProps) {
  ...
  // Auto-open detail panel with first person when people are found
  useEffect(() => {
    if (
      ...
      // Open detail panel with first person
      openDetailPanel(response.people[0]);
    }
  }, [response, openDetailPanel]);
  ...
  if (
    ...
    return (
      <div className="space-y-6">
        ...
        {/* User Preview Cards - Horizontal Scrollable */}
        <div className="overflow-x-auto pb-4">
          <div className="flex gap-3 min-w-max">
            {response.people.map((person) => (
              <div key={person.id} className="flex-shrink-0 w-72">
                <UserPreviewCard
                  user={person}
                  isSelected={selectedUser?.id === person.id}
                  onClick={() => openDetailPanel(person)}
                />
              </div>
            ))}
          </div>
