...
const ChatItem: React.FC<ChatItemProps> = ({
  ...
  const displayContent = isLongUserMessage && !expanded
    ? content.slice(0, CHAR_LIMIT).trim() + '...'
    : content;

  return (
    <div className={`flex items-start gap-4 ${role === 'assistant' ? 'bg-muted/30' : ''} p-4`}>
      {/* Avatar */}
        ...
        {isLoading ? (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-primary animate-pulse" style={{ borderRadius: '0px' }} />
              <span className="text-sm text-muted-foreground">AI is searching for people...</span>
            </div>
          ...
          <div>
            ...
            {chatResponse?.query_type === 'people_search' && chatResponse.people && chatResponse.people.length > 0 && (
              <div className="mt-6">
                ...
                {chatResponse.explanation && (
                  <div className="mt-4 p-3 bg-muted/50 border border-muted" style={{ borderRadius: '0px' }}>
                    <p className="text-xs text-muted-foreground">
                      <strong>Search explanation:</strong> {chatResponse.explanation}
                    </p>
                  </div>
                )}
              </div>
            ...
            {chatResponse?.query_type === 'people_search' && chatResponse.people && chatResponse.people.length === 0 && (
              <div className="mt-6">
                <div className="flex flex-col items-center justify-center p-8 bg-muted/30 border border-muted text-center" style={{ borderRadius: '0px' }}>
            ...
            {chatResponse?.query_type === 'general_question' && chatResponse.message && (
              <div className="mt-4 p-4 bg-muted/30 border border-muted" style={{ borderRadius: '0px' }}>
